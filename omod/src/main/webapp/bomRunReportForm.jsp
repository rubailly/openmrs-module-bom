							            	...
							            	<td colspan="2">
												...
												<c:if test="${!empty report.existingRequestUuid}">
													<span style="padding-left:20px;">
														<a onclick="return confirm('<spring:message code="reporting.reportHistory.confirmDelete"/>');" href="../deleteReportRequest.form?uuid=${report.existingRequestUuid}">
						...
						<fieldSet>
							...
							<div style="padding:10px;">
								<h4>
									...
									&nbsp;&nbsp;
									<a href="${pageContext.request.contextPath}/module/bom/bomReportHistory.form?reportDefinition=${report.reportDefinition.uuid}">
										(<spring:message code="reporting.viewAll"/>)
									</a>
								</h4>
								<div style="padding:5px 0px 5px 10px;">
									<openmrs:portlet url="bomReportRequests" id="completedRequests" moduleId="bom" parameters="reportId=${report.reportDefinition.id}|status=SAVED,COMPLETED,FAILED|mostRecentNum=1"/>
								</div>
								<h4><spring:message code="reporting.Report.currentlyRunning"/></h4>
								<div style="padding:5px 0px 5px 10px;">
									<openmrs:portlet url="bomReportRequests" id="runningRequests" moduleId="bom" parameters="reportId=${report.reportDefinition.id}|status=PROCESSING"/>
								</div>
								<h4><spring:message code="reporting.Report.currentlyQueued"/></h4>
								<div style="padding:5px 0px 5px 10px;">
									<openmrs:portlet url="bomReportRequests" id="queuedRequests" moduleId="bom" parameters="reportId=${report.reportDefinition.id}|status=REQUESTED"/>
								</div>
								<h4><spring:message code="reporting.Report.currentlyScheduled"/></h4>
								<div style="padding:5px 0px 5px 10px;">
									<openmrs:portlet url="bomReportRequests" id="scheduledRequests" moduleId="bom" parameters="reportId=${report.reportDefinition.id}|status=SCHEDULED"/>
								</div>
							</div>
						</fieldSet>
					</td>
